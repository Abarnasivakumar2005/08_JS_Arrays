...
// 3) Create a showItem function
var showItem = function (player, itemNumber) {
  if (itemNumber > 0 && itemNumber <= player.items.length) {
      console.log(player.name + " has: " + player.items[itemNumber - 1]);
  } else {
      console.log("Item number is out of range.");
  }
};

// Example usage of showItem
showItem(player1, 1); // Displays the first item
showItem(player1, 2); // Displays the second item

// 4) Create an addItem function
var addItem = function (player, itemTitle) {
  player.items.push(itemTitle);
  console.log(itemTitle + " has been added to " + player.name + "'s inventory.");
};

// Example usage of addItem
addItem(player1, "a magical stone");
showPlayerInfo(player1, "#");
